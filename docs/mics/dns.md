...
![基本工作流程](https://picxyxsw.oss-cn-hangzhou.aliyuncs.com/20231006233612.png)
...
这种方法会让dns递归服务器发出多次请求，其中①是递归查询，后继的三个查询是迭代查询。

### 递归查询工作流程

![递归查询工作流程](https://picxyxsw.oss-cn-hangzhou.aliyuncs.com/20231015181102.png)
...
当然监听这种地址需要在**网卡上创建一个新ip**

对于op来说 可以在web管理界面 选择网络 新建一个接口 区域lan 设备br-lan 地址为`172.16.3.53`

对于监听53端口、你的本地 dnsmasq 可能会劫持这个端口

需要关闭 dnsmasq

打开 openwrt 的web管理界面 `网络`-`接口`-`DHCP/DNS`-`高级设置`-`DNS 服务器端口` 改成0

这样dnsmasq就不会监听你的53端口了
...
目前所有的dns注册都在 [Registry](https://github.com/dn-11/registry) 里面，生成的 zone 文件在 [metadata里的zone](https://raw.githubusercontent.com/dn-11/metadata/main/dn11.zone)
...
### 这是装pdns的第二种方法2.docker拉镜像 装pdns-admin web界面管理
...
同样的 启动需要你br-lan网卡里加一个 `172.16.3.13` 这个 ip 地址
